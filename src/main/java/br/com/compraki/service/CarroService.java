...
public class CarroService {
	...
	@Transactional
	public Carro salvarCarroIntermiediario(Carro carro) throws NegocioException {
		try {
			ModeloVeiculo modelo = this.modelosVeiculos.saveAndFlush(carro.getModelo());
			carro.setModelo(modelo);
			carro.setAcessorios(getAcessoriosGerenciados(carro));
			carro.setUsuario(this.usuarios.findOne(carro.getUsuario().getCodigo()));
			return this.carros.saveAndFlush(carro);
		} catch (Exception e) {
			throw new NegocioException(e.getMessage());
		}
	}
